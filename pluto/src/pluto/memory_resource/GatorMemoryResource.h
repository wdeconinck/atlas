...
class GatorMemoryResource : public std::pmr::memory_resource {
public:

	GatorMemoryResource() : 
		GatorMemoryResource(std::pmr::get_default_resource()) {}

	GatorMemoryResource(std::pmr::memory_resource* upstream) :
		upstream_(upstream) {
		init(GatorOptions());
	}

	GatorMemoryResource(std::unique_ptr<std::pmr::memory_resource>&& upstream) :
	...
	GatorMemoryResource(const GatorOptions& options, std::pmr::memory_resource* upstream) :
		upstream_(upstream) {
		init(options);
	}

	GatorMemoryResource(const GatorOptions& options, std::unique_ptr<std::pmr::memory_resource>&& upstream) :
		owned_upstream_(std::move(upstream)),
		upstream_(owned_upstream_.get()) {
		init(options);
	}
