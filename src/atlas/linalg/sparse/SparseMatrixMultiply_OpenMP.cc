...
template <typename SourceValue, typename TargetValue>
void SparseMatrixTransposeMultiply<backend::omp, Indexing::layout_left, 1, SourceValue, TargetValue>::apply(
    const SparseMatrix& W, const View<SourceValue, 1>& src, View<TargetValue, 1>& tgt, const Configuration& ) {
    using Value      = TargetValue;
    const auto outer = W.outer();
    const auto index = W.inner();
    const auto value = W.data();
    const idx_t rows = static_cast<idx_t>( W.rows() );

    ATLAS_ASSERT( src.shape( 0 ) >= W.rows() );
    ATLAS_ASSERT( tgt.shape( 0 ) >= W.cols() );

    atlas_omp_parallel_for( idx_t r = 0; r < rows; ++r ) { tgt[r] = 0.; }

    atlas_omp_parallel_for( idx_t r = 0; r < rows; ++r ) {
        for ( idx_t c = outer[r]; c < outer[r + 1]; ++c ) {
            idx_t n = index[c];
            Value w = static_cast<Value>( value[c] );
            tgt[n] += w * src[r];
        }
    }
}
...
template <typename SourceValue, typename TargetValue>
void SparseMatrixTransposeMultiply<backend::omp, Indexing::layout_left, 2, SourceValue, TargetValue>::apply(
    const SparseMatrix& W, const View<SourceValue, 2>& src, View<TargetValue, 2>& tgt, const Configuration& ) {
    using Value      = TargetValue;
    const auto outer = W.outer();
    const auto index = W.inner();
...
template <typename SourceValue, typename TargetValue>
void SparseMatrixTransposeMultiply<backend::omp, Indexing::layout_left, 3, SourceValue, TargetValue>::apply(
    const SparseMatrix& W, const View<SourceValue, 3>& src, View<TargetValue, 3>& tgt, const Configuration& config ) {
    if ( src.contiguous() && tgt.contiguous() ) {
        // We can take a more optimized route by reducing rank
        auto src_v = View<SourceValue, 2>( src.data(), array::make_shape( src.shape( 0 ), src.stride( 0 ) ) );
        auto tgt_v = View<TargetValue, 2>( tgt.data(), array::make_shape( tgt.shape( 0 ), tgt.stride( 0 ) ) );
        SparseMatrixTransposeMultiply<backend::omp, Indexing::layout_left, 2, SourceValue, TargetValue>::apply(
    ...
    atlas_omp_parallel_for( idx_t r = 0; r < rows; ++r ) {
        for ( idx_t c = outer[r]; c < outer[r + 1]; ++c ) {
            idx_t n       = index[c];
            const Value w = static_cast<Value>( value[c] );
            for ( idx_t k = 0; k < Nk; ++k ) {
                for ( idx_t l = 0; l < Nl; ++l ) {
                    tgt( n, k, l ) += w * src( r, k, l );
                }
            }
        }
    }
...
// SparseMatrixTransposeMultiply

#define EXPLICIT_TEMPLATE_INSTANTIATION( TYPE )                                                              \
    template struct SparseMatrixTransposeMultiply<backend::omp, Indexing::layout_left, 1, TYPE const, TYPE>; \
    template struct SparseMatrixTransposeMultiply<backend::omp, Indexing::layout_left, 2, TYPE const, TYPE>; \
    template struct SparseMatrixTransposeMultiply<backend::omp, Indexing::layout_left, 3, TYPE const, TYPE>;
