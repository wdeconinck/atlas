...
static const std::vector<std::function<void( double[] )>> tileRotate = {
    []( double xyz[] ) {tile1Rotate(xyz); }, []( double xyz[] ) {tile2Rotate(xyz); },
    []( double xyz[] ) {tile3Rotate(xyz); }, []( double xyz[] ) {tile4Rotate(xyz); },
    []( double xyz[] ) {tile5Rotate(xyz); }, []( double xyz[] ) {tile6Rotate(xyz); }};
...
static const std::vector<std::function<void( double[] )>> tileRotateInverse = {
    []( double xyz[] ) {tile1RotateInverse(xyz); }, []( double xyz[] ) {tile2RotateInverse(xyz); },
    []( double xyz[] ) {tile3RotateInverse(xyz); }, []( double xyz[] ) {tile4RotateInverse(xyz); },
    []( double xyz[] ) {tile5RotateInverse(xyz); }, []( double xyz[] ) {tile6RotateInverse(xyz); },
};
...
idx_t CubedSphereProjectionBase::tileFromLonLat( const double crd[] ) const {
    ...
    if ( lon >= 1.75 * M_PI || lon < 0.25 * M_PI ) {
        ...
        if ( is_same( lon, -0.25 * M_PI ) && is_same( lat, cornerLat ) ) {
            t = 0;
        }
    }
    ...
    if ( lon >= 0.75 * M_PI && lon < 1.25 * M_PI ) {
        ...
        // extra point corner point
        if ( is_same( lon, 0.75 * M_PI ) && is_same( lat, -cornerLat ) ) {
            t = 1;
        }
    }
