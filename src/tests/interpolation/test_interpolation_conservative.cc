...
CASE( "test_interpolation_conservative" ) {
    ...
    for ( idx_t scell = 0; scell < src_vals.size(); ++scell ) {
        ...
        for ( idx_t nid = 0; nid < src_neighbour_cells.size(); ++nid ) {
            ...
            if ( ncell != scell && nncell != scell ) {
                ...
                dual_area +=
                    CSPolygon( {src_csp[ncell].centroid(), src_csp[nncell].centroid(), src_csp[scell].centroid()} )
                        .area();
                grad = PointXYZ::add(
                    grad,
                    PointXYZ::mul( PointXYZ::cross( src_csp[ncell].centroid(), src_csp[nncell].centroid() ), coeff ) );
            }
        }
        ...
        for ( idx_t icell = 0; icell < iparam.centroids.size(); ++icell ) {
            ...
            tgt_vals( iparam.cell_id[icell] ) +=
                ...
                ( src_vals( scell ) + PointXYZ::dot( grad, iparam.centroids[icell] - src_csp[scell].centroid() ) );
        }
    }
    for ( idx_t tcell = 0; tcell < tgt_vals.size(); ++tcell ) {
    ...
    for ( idx_t scell = 0; scell < src_vals.size(); ++scell ) {
        ...
        for ( idx_t icell = 0; icell < iparam.weights.size(); ++icell ) {
            scell_area += iparam.weights[icell];
        }
        Log::info() << " scell, scell_area, diff: " << src_csp[scell].area() << " " << scell_area << " "
                    << src_csp[scell].area() - scell_area << "\n";
        err += std::abs( src_csp[scell].area() - scell_area );
    }
