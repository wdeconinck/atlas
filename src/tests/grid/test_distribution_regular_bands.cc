...
CASE( "test_nbands" ) {
    ...
    grid::Distribution dist( grid, atlas::util::Config( "type", "checkerboard" ) | Config( "nbands", nproc ) );
...
CASE( "test_regular_bands" ) {
    ...
    for ( auto gridname : gridnames ) {
        SECTION( gridname ) {
            ...
            grid::Distribution checkerboard( grid, grid::Partitioner( "checkerboard", Config( "nbands", nproc ) ) );
